...
class EdgeSelectionStrategy:
    ...
    def _estimate_success_probability(self, difficulty: float, threshold: float) -> float:
        ...
        """
        # Normalize difficulty (assume 1-10 scale)
        norm_difficulty = difficulty / 10.0
        ...
        threshold_penalty = (threshold - 0.5) * 0.3
    ...
    def _estimate_bell_pair_cost(self, difficulty: float, threshold: float) -> float:
        ...
        """
        # Base cost: 2 pairs minimum
        base_cost = 2.0
        
        # Difficulty scaling (1-10 scale)
        difficulty_factor = difficulty / 10.0
        
        # Threshold scaling
        threshold_factor = (threshold - 0.5) * 2.0
        
        # Combined cost estimate
        estimated_cost = base_cost + difficulty_factor * 3.0 + threshold_factor * 2.0
        
        # Clamp to valid range [2, 8]
        return max(2.0, min(8.0, estimated_cost))
