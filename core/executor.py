...
Supports both legacy agent (agent.py) and new LangGraph agent (langgraph_deterministic_agent.py).
"""
...
class GameExecutor:
    ...
    def create_agent(
        ...
        Args:
            ...
            use_langgraph: Use LangGraph agent instead of legacy agent
            
        Returns:
            Configured agent (QuantumNetworkAgent or LangGraphQuantumAgent)
        """
        if use_langgraph:
            ...
            if config:
                # Convert AgentConfig to LangGraphAgentConfig if needed
                if isinstance(config, AgentConfig):
                    lg_config = LangGraphAgentConfig(
                        utility_weight=config.utility_weight,
                        difficulty_weight=config.difficulty_weight,
                        cost_weight=config.cost_weight,
                        success_prob_weight=config.success_prob_weight,
                        ...
                        prefer_dejmps=config.prefer_dejmps,
                        adaptive_risk=config.adaptive_risk,
                        adaptive_pairs=config.adaptive_pairs
                    )
                    self.agent = LangGraphQuantumAgent(self.client, lg_config)
                else:
                    self.agent = LangGraphQuantumAgent(self.client, config)
            elif agent_type == "aggressive":
                self.agent = create_aggressive_langgraph_agent(self.client)
            elif agent_type == "conservative":
                self.agent = create_conservative_langgraph_agent(self.client)
            else:
                self.agent = create_default_langgraph_agent(self.client)
            
            print(f"✓ Created {agent_type} LangGraph agent")
        else:
            # Legacy agent
            if config:
            ...
            print(f"✓ Created {agent_type} agent (legacy)")
