...
def create_recursive_distillation_circuit(num_bell_pairs: int) -> Tuple[QuantumCircuit, int]:
    ...
    ⚠️  EXPERIMENTAL: This function is not production-ready.
    
    KNOWN LIMITATION: Flag bit logic is incomplete. The flag is hardcoded to qubit 0
    instead of computing the XOR of all measurement results. This means post-selection
    may not work correctly for this protocol.
    
    For production use, implement proper flag computation or use BBPSSW/DEJMPS instead.
    ...
    # TODO: Implement proper flag bit computation (XOR of all measurements)
    # Currently using simplified flag (first qubit only)
    flag_bit = 0  # INCOMPLETE: Should be XOR of all ancilla measurements
